...
class JobExchangeListVirtualHostMapperTest extends TestCase
{
    ...
    public function testBasicJob() {
        ...
        $mapper = new JobExchangeListVirtualHostMapper($this->config);
        $mapResult = $mapper->map($job);

        $this->assertEquals(Method::GET, $mapResult->getMethod()->getValue());
        $this->assertEquals('exchanges/'.urlencode($virtualHost), $mapResult->getUrl());
    }
}
